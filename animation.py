...
class AnimationController():
    ...
    def setup_animation(self):
        ...
        for animation in animation_steps:
            ...
            for _ in range(animation):
                temp_image_list.append(sprite_sheet.get_image(step_counter, 40, 40, 5, self.BLACK)) #step_counter is the x-cords where the current frame of spritesheet starts 
                #get_image is from spritesheet module where step_counter will be 1, 2, 3 and it will affect which image is gotten :P
                step_counter += 1
    ...
    def update_animation(self, screen, player_x, player_y):
        ...
        if (current_time - self.last_update) >= self.animation_cooldown: # calculates the time elapsed since the last update between the current time. #example the d animation is at 5 sec while last updt is at 2 sec it will run
            self.last_update = current_time #when it ran it update the current time as the newer last_update
            if self.frame >= len(self.animation_list[self.action]): #checks if the animation for each movement is finished, if finished it will return it black to idle animation which is frame 0
                # if my action is 0 it plays the first list in animation list which is idle , where if action is 1 it playes the second list of animation in animation_list
                self.frame = 0
                

        # Update the animation using player coordinates
